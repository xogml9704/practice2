...
Y = tf.keras.layers.Dense(6, activation='sigmoid')(H)

model = tf.keras.models.Model(X, Y)
model.compile(loss='categorical_crossentropy', metrics='accuracy')

early_stopping = tf.keras.callbacks.EarlyStopping(monitor='accuracy', patience=5)
history = model.fit(독립, 종속, validation_split=0.2,  epochs=1, batch_size = 64, callbacks=[early_stopping])
#%%
...
print(test_독립.shape)
#%%
model.save('D:\\code\\model\\lenet5_1_model_test.h5')
